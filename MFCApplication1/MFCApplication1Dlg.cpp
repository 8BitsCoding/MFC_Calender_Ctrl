...
void CMFCApplication1Dlg::OnMcnSelchangeMyCalendar(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	SYSTEMTIME cur_time;
	m_my_calendar.GetCurSel(&cur_time);		// 달력의 현재 날짜정보를 넘긴다.

	wchar_t * p_week_str[7] = { L"일",L"월", L"화", L"수", L"목", L"금", L"토" };
	CString str;
	str.Format(L"%d년 %d월 %d일 [%s요일]", cur_time.wYear, cur_time.wMonth, cur_time.wDay, p_week_str[cur_time.wDayOfWeek]);
	// cur_time.wDayOfWeek = 요일정보
